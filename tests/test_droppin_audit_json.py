...
def audit_api(tmp_path, monkeypatch):
    ...
    with connect() as conn:
        conn.execute('CREATE TABLE site_audits (audit_id TEXT, url TEXT, status TEXT, overall_score REAL, report_json TEXT)')
...
def test_polling_contract(audit_api, status, score, report, expected):
    ...
    with connect() as conn:
        conn.execute('INSERT INTO site_audits VALUES (?, ?, ?, ?, ?)',
                     ('audit-1', 'https://example.com', status, score, json.dumps(report)))
