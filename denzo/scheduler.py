...
def configure(tenant_id, enabled, zone='UTC', hour=9, now=None):
    ...
    db = get_db()
    try:
        db.execute('''INSERT INTO schedules(tenant_id,enabled,timezone,hour,next_run_at)
...
def _run_due(tenant_id):
    ...
    db = get_db()
    try:
        pages = [dict(r) for r in db.execute('SELECT * FROM pages WHERE tenant_id=?', (tenant_id,))]
        client = db.execute('SELECT publisher_type FROM clients WHERE tenant_id=?', (tenant_id,)).fetchone()
        if any(publishable(p) for p in pages):
            agent = 'WordPress Publisher' if client['publisher_type']=='wordpress' else 'GitHub Publisher'
            ...
            db.execute('''INSERT INTO settings(tenant_id,key,value) VALUES (?,'generation_batch_size','1')
                ON CONFLICT(tenant_id,key) DO UPDATE SET value='1' ''', (tenant_id,))
            db.execute("""UPDATE agents SET status='idle',run_count=0 WHERE tenant_id=?
              ...
              AND status NOT IN ('working','starting')""", (tenant_id,))
            db.commit()
    finally:
        db.close()
...
def tick(now=None, verify=True):
    ...
    try:
        ...
        due = [dict(r) for r in db.execute('SELECT * FROM schedules WHERE enabled=1 AND next_run_at<=?', (now.isoformat(),))]
        # Short claim lease lets a crashed scheduler resume without claiming the same schedule twice.
        for row in due:
            db.execute('UPDATE schedules SET next_run_at=? WHERE tenant_id=?', ((now+timedelta(minutes=5)).isoformat(),row['tenant_id']))
        db.commit()
    finally:
        db.close()
    results = []
    for row in due:
        try:
            ...
            logging.getLogger(__name__).exception('Schedule failed: %s', row['tenant_id'])
            result = 'error'
        upcoming = now+timedelta(minutes=5) if result in ('busy','error') else next_daily(now,row['timezone'],row['hour'])
        db = get_db()
        try:
            db.execute('UPDATE schedules SET next_run_at=?,last_run_at=?,last_result=? WHERE tenant_id=? AND enabled=1',
