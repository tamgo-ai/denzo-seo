...
def site_base_url(ctx):
    return normalize_base(
        ctx.pages_domain
        or (ctx.wp_url if ctx.publisher_type == "wordpress" else "")
        or ctx.domain
        or ctx.website_url
    )
...
def public_page_url(page, ctx):
    ...
    return (
        f"{base}/en/{folder}/{slug}"
        if ctx.github_format == "nextjs"
        else f"{base}/{folder}/{slug}.html"
    )
...
def quality_document(content, page, ctx):
    ...
    content = sanitize_fragment(content)
    page = dict(page)
