...
def deep_image_audit(url: str, html: str, domain: str, base_page_url: str = None, framework: dict = None) -> dict:
    ...
    if alt_generic:
        ...
        findings.append({"severity":"medium","module":"images","title":f"{len(alt_generic)} images have generic/low-quality alt text","detail":f"Examples: {examples}. Alt text like 'auto body repair' or 'car' provides zero descriptive value. Google Images ranks based on alt text relevance + surrounding content. Generic alt text = invisible in image search.","fix":"Rewrite alt text to be specific and descriptive. Describe exactly what the image shows. Each image should have UNIQUE, contextual alt text.","impact":"Reduced Google Images visibility."})
        score -= 8
