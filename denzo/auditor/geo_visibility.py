...
def analyze_geo_visibility(url: str, html: str, domain: str, industry_profile: dict = None) -> dict:
    ...
    entity_count = sum(entity.values())
    missing_entity = [k for k,v in entity.items() if not v]

    if entity_count >= 5:
        findings.append({"severity":"pass","module":"geo","title":f"Strong entity signals: {entity_count}/7 present","detail":f"Present: {[k for k,v in entity.items() if v]}. AI models have multiple confidence signals to identify and cite this business.","fix":None})
    elif entity_count < 3:
        findings.append({"severity":"high","module":"geo","title":f"Weak entity signals: only {entity_count}/7 — AI may not trust this business","detail":f"Missing: {missing_entity}. AI models need clear, structured entity signals (phone, scale, certifications, history, address, guarantees) to confidently cite a business.","fix":"Ensure the following are in visible body text (not just schema or footer): phone number, number of locations (if multi-site), certifications/credentials, founding year, physical address, service guarantees."})
        score -= 15
    ...
    if any(b['yours'] < b['avg'] for b in benchmarks.values()):
        ...
        findings.append({"severity":"medium","module":"geo","title":f"Below industry average in {len(below)}/{len(benchmarks)} GEO metrics","detail":"Benchmark comparison for {industry.replace('_', ' ')} / local service sites:\n" + '\n'.join(benchmark_lines),"fix":"Prioritize metrics marked with ✗ or ⚠. Highest-ROI: FAQ content, structured lists, definition block."})
        score -= 10
