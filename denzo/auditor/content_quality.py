...
def analyze_content_quality(url: str, html: str, domain: str, industry_profile: dict = None) -> dict:
    ...
    if word_count < 400:
        findings.append({"severity":"high","module":"content","title":f"Severely thin content: {word_count} words","detail":f"At {word_count} words, this page cannot cover the topic in enough depth to compete. Competitive pages for commercial queries tend to be substantially longer. Google's Helpful Content System favors comprehensive, original, useful content.","fix":"Expand to 1,000+ words. Structure with H2/H3 sections: detailed service descriptions, process, FAQ, about/credentials, testimonials, and location-specific content. Add original data points only this business can provide (years in business, certifications, before/after results).","impact":"Cannot compete for mid-to-high difficulty keywords."})
        score -= 20
    elif word_count < 800:
        findings.append({"severity":"medium","module":"content","title":f"Thin content: {word_count} words","detail":f"Below the depth expected for competitive ranking. Competitive pages for commercial queries tend to be substantially longer.","fix":"Expand to 1,000+ words with additional sections: FAQ, detailed service descriptions, case studies, and location-specific content.","impact":"Limited ability to rank for competitive terms."})
        score -= 10
    ...
    if word_count > 500 and rich_elements < 2:
        findings.append({"severity":"medium","module":"content","title":"Text-heavy with no visual breaks — poor engagement","detail":f"{word_count} words with only {rich_elements} rich elements. Dense text without visual breaks is harder to scan and can hurt engagement.","fix":"Add: relevant images, a data table for comparison, customer quotes in blockquotes, or an embedded video. Visual elements can improve engagement and time-on-page."})
        score -= 5
