...
def create_app():
    ...
    def _tenant_boundary():
        # Defence in depth: newly added tenant routes cannot omit authorization.
        from flask import abort, jsonify, redirect, session, url_for
        from denzo.auth import can_access_tenant
        tenant_id = (request.view_args or {}).get('tenant_id')
        if tenant_id and not can_access_tenant(tenant_id):
            if request.path.startswith('/api/'):
                return jsonify(error='Access denied'), 403 if session.get('user_id') else 401
...
def _reset_stale_agents():
    ...
    db = get_db()
    try:
        db.execute("""UPDATE agents SET status='error',current_task='Interrupted legacy execution; review and retry'
          WHERE status IN ('working','starting') AND NOT EXISTS (
          SELECT 1 FROM agent_jobs j WHERE j.tenant_id=agents.tenant_id AND j.agent_name=agents.name
